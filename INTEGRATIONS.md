...
**sigsurvey-rf** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `sigsurvey-rf` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
sigsurvey-rf ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
sigsurvey-rf ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
sigsurvey-rf ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `sigsurvey-rf` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="sigsurvey-rf") for rec in records]   # records = your JSON output
