...
def to_console(r: ScanResult) -> str:
    ...
    for f in r.findings[:100]:
        hdr.append(f"  {ICON[f.severity]} [{f.severity.value.upper():<10}] {f.id:<14} {f.title}")
        if f.location:
            hdr.append(f"      📍 {f.location}")
        if f.nist_800_53:
            hdr.append(f"      📋 NIST 800-53: {f.nist_800_53}")
        if f.disa_stig:
            hdr.append(f"      🛡  STIG: {f.disa_stig}")
        if f.mitre_attack:
            hdr.append(f"      🎯 ATT&CK: {f.mitre_attack}")
        if f.remediation:
            hdr.append(f"      💡 {f.remediation}")
    hdr.append("═" * 70)
    hdr.append(f"  {r.classification_placeholder}")
    hdr.append("═" * 70)
    return "\n".join(hdr)
